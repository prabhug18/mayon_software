#!/usr/bin/env node
// CommonJS Puppeteer PDF renderer (use .cjs to avoid ES module mode when package.json has "type":"module")
const fs = require('fs');
...
async function main(){
  ...
  if (argv.length < 2) {
    console.error('Usage: node puppeteer-render.cjs <url> <outputPath>');
    process.exit(2);
  }
